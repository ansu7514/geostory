...
function countMapped(data, c) {
  ...
  return data.filter(r => {
    const la = parseFloat(r[c.lat]), lo = parseFloat(r[c.lng])
...
export function useGeoData() {
  ...
  const finalize = useCallback(async (data, name, ext, detectedCols) => {
    setError(null)
    let c = { ...detectedCols }
    let finalData = [...data]

    if ((ext === 'geojson' || ext === 'json') && data.some(r => r._lat !== undefined)) {
      c.lat = '_lat'; c.lng = '_lng'
    }
  ...
  const loadDirect = useCallback((data, name, detectedCols) => {
    finalize(data, name, 'csv', detectedCols)
  }, [finalize])

  const reset = useCallback(() => {
    setRows([]); setCols({ lat: null, lng: null, addr: null })
    setFileName(''); setMappedCount(0); setGeocoding(false)
    setGeocodeProgress({ done: 0, total: 0 }); setFileType(''); setError(null)
  }, [])

  return { rows, cols, fileName, mappedCount, geocoding, geocodeProgress, fileType, error, processFile, loadDirect, reset }
}
