...
export const CAT_COLORS = {
  '랜드마크': '#4f7cff', '광장': '#4ade80', '관광': '#fbbf24', '유적': '#f87171',
  '문화': '#c084fc', '공원': '#34d399', '상업': '#fb923c', '교통': '#60a5fa',
  '음식': '#f472b6', '숙박': '#a78bfa', '의료': '#2dd4bf', '교육': '#86efac',
  default: '#8892b0',
}
...
function ImageOverlayPanel({ map, onImageAdded, resetTrigger }) {
  ...
  const loadGeoTIFF = async (file) => {
    ...
    try {
      ...
      // CRS 감지 및 좌표 변환
      const isWebMercator = raster.projection === 3857 ||
        (raster.xmin > 1000 || raster.xmin < -1000)
  ...
  const placeImageOverlay = () => {
    const n = parseFloat(bounds.n), s = parseFloat(bounds.s)
    const e = parseFloat(bounds.e), w = parseFloat(bounds.w)
    if ([n, s, e, w].some(isNaN)) {
      alert('유효한 좌표를 입력하세요')
      return
    }
    const leafletBounds = [[s, w], [n, e]]
  ...
  const removeOverlay = (id) => {
    const layer = layerRefs.current[id]
    if (layer) { map.removeLayer(layer); delete layerRefs.current[id] }
  ...
  const s = {
    ...
    title: { fontSize: 10, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 6, fontWeight: 600 },
    dropzone: (active) => ({
      border: `1.5px dashed ${active ? 'var(--accent)' : 'var(--border)'}`,
      borderRadius: 8, padding: '12px 10px', textAlign: 'center', cursor: 'pointer',
      background: active ? 'rgba(79,124,255,0.07)' : 'transparent', transition: 'all 0.2s',
      fontSize: 11, color: 'var(--text2)',
    }),
    input: { width: '100%', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 5, padding: '5px 8px', fontSize: 11, color: 'var(--text)', fontFamily: 'monospace', outline: 'none' },
    btn: (primary) => ({ padding: '5px 10px', borderRadius: 6, fontSize: 11, cursor: 'pointer', border: `1px solid ${primary ? 'var(--accent)' : 'var(--border)'}`, background: primary ? 'var(--accent)' : 'var(--bg3)', color: primary ? '#fff' : 'var(--text)', fontFamily: 'inherit' }),
  }
  ...
  return (
    <div style={s.section}>
      ...
      >
        ...
        <div style={{ fontSize: 10, marginTop: 3, color: 'var(--text3)' }}>JPG · PNG · TIFF · GeoTIFF</div>
        <input ref={fileRef} type="file" accept=".jpg,.jpeg,.png,.webp,.tif,.tiff" style={{ display: 'none' }} onChange={e => { if (e.target.files[0]) loadFile(e.target.files[0]) }} />
      </div>
      ...
      {placing && (
        <div style={{ marginTop: 8, background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 8, padding: 10 }}>
          <div style={{ fontSize: 11, color: 'var(--warn)', marginBottom: 8 }}>📍 "{placing.name}" 위치 설정</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5, marginBottom: 6 }}>
            {[['n', '북위 (상단)'], ['s', '남위 (하단)'], ['e', '동경 (우측)'], ['w', '서경 (좌측)']].map(([k, label]) => (
              <div key={k}>
                <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 2 }}>{label}</div>
                <input style={s.input} placeholder="37.xxx" value={bounds[k]} onChange={e => setBounds(p => ({ ...p, [k]: e.target.value }))} />
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 5 }}>
            <button onClick={useMapBounds} style={s.btn(false)}>현재 지도 범위</button>
            <button onClick={placeImageOverlay} style={s.btn(true)}>지도에 올리기</button>
            <button onClick={() => setPlacing(null)} style={{ ...s.btn(false), color: 'var(--danger)', borderColor: 'var(--danger)' }}>✕</button>
          </div>
        </div>
      )}
      ...
      {/* Overlay list */}
      {overlays.map(o => (
        <div key={o.id} style={{ marginTop: 6, background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 7, padding: '8px 10px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
            <div style={{ fontSize: 11, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              <span style={{ background: 'var(--bg)', border: '1px solid var(--border)', padding: '1px 4px', borderRadius: 3, fontSize: 9, marginRight: 5, color: 'var(--text2)' }}>{o.type}</span>
              {o.name}
            </div>
            <div style={{ display: 'flex', gap: 4, flexShrink: 0, marginLeft: 6 }}>
              <button onClick={() => toggleOverlay(o.id)} style={{ ...s.btn(false), padding: '2px 7px', fontSize: 10 }}>{o.visible ? '숨김' : '표시'}</button>
              <button onClick={() => removeOverlay(o.id)} style={{ ...s.btn(false), padding: '2px 7px', fontSize: 10, color: 'var(--danger)' }}>✕</button>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 10, color: 'var(--text2)', width: 28 }}>{Math.round(o.opacity * 100)}%</span>
          </div>
        </div>
      ))}
...
export default function MapView({ rows, cols, vizType, layerType, onLayerChange, resetTrigger }) {
  ...
  useEffect(() => {
    ...
    tileRef.current = L.tileLayer(LAYERS[layerType] || LAYERS.dark, { attribution: '', maxZoom: 19 }).addTo(m)
  }, [layerType])
  ...
  useEffect(() => {
    ...
    const bounds = []
    rows.forEach(row => {
      const lat = parseFloat(row[cols.lat]), lng = parseFloat(row[cols.lng])
      ...
      if (vizType === 'heatmap') {
        const c = L.circleMarker([lat, lng], { radius: 22, fillColor: color, color: 'transparent', fillOpacity: 0.12 }).addTo(m)
      ...
      const entries = Object.entries(row).filter(([k]) => !k.startsWith('_')).slice(0, 7)
      mk.bindPopup(`<div style="min-width:170px">${entries.map(([k, v]) =>
  ...
  // Legend
  const cats = [...new Set(rows.map(r => r.category || r.type || r.분류 || r.카테고리).filter(Boolean))]
  ...
  return (
    <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Layer controls */}
      <div style={{ position: 'absolute', bottom: 14, left: 14, zIndex: 1000, display: 'flex', gap: 5 }}>
        {['dark', 'sat', 'street'].map(t => (
          <button key={t} onClick={() => onLayerChange(t)} style={{
      ...
      {cats.length >= 2 && (
        <div style={{
          ...
          border: '1px solid var(--border)', borderRadius: 9, padding: '10px 13px', minWidth: 130,
        }}>
          <div style={{ fontSize: 10, color: 'var(--text2)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.5px' }}>범례</div>
          {cats.slice(0, 8).map(c => (
            <div key={c} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 11, marginBottom: 3 }}>
              <div style={{ width: 9, height: 9, borderRadius: '50%', background: CAT_COLORS[c] || CAT_COLORS.default, flexShrink: 0 }} />
              {c}
            </div>
          ))}
        </div>
      ...
      {mapReady && (
        <div style={{
          position: 'absolute', bottom: 50, right: 14, zIndex: 1000,
          background: 'rgba(26,29,39,0.96)', backdropFilter: 'blur(10px)',
          border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px',
          width: 260, maxHeight: '60vh', overflowY: 'auto',
        }}>
          <ImageOverlayPanel map={mapInstanceRef.current} resetTrigger={resetTrigger} />
        </div>
      )}
    </div>
  )
